...
samples_BunnysTextureSwitch.prototype = $extend(samples_Sample.prototype,{
	run: function() {
		...
		this.peoteView.setTexture({ texture : 1, w : 512, h : 512});
		this.peoteView.imageCache.setImage({ image : 1, texture : 1, preload : true, filename : "assets/peote_tiles_flowers_alpha.png"});
		this.peoteView.setProgram({ program : 1, texture : 1, vshader : "assets/ping-pong_android.vert"});
		this.peoteView.setTexture({ texture : 2, w : 512, h : 512});
		this.peoteView.imageCache.setImage({ image : 2, texture : 2, preload : true, filename : "assets/peote_tiles_bunnys.png"});
		this.peoteView.setProgram({ program : 2, texture : 2, vshader : "assets/ping-pong_android.vert"});
		this.peoteView.setTexture({ texture : 3, w : 512, h : 512});
		this.peoteView.imageCache.setImage({ image : 3, texture : 3, preload : true, filename : "assets/peote_font_white.png"});
		...
		this.peoteView.setDisplaylist({ displaylist : 4, type : 8, maxElements : 200, x : 460, y : 0, w : 1, h : 1, renderBackground : true, a : 0.8, r : 0.25, z : 1});
		this.peoteView.setDisplaylist({ displaylist : 5, type : 8, maxElements : 12, x : 8, y : 4, w : 1, h : 1, renderBackground : true, a : 0.75, r : 0.25, z : 1});
	...
	,spawnBunnys: function() {
		...
		if(!this.pause) haxe_Timer.delay($bind(this,this.spawnBunnys),Math.floor(1 / this.spawn_time * 1000));
	}
	,refreshFPS: function() {
	...
	,render: function(renderer) {
		this.peoteView.render(haxe_Timer.stamp() - this.startTime,this.width,this.height,this.mouse_x,this.mouse_y,1,0,0);
		this.frames++;
	}
	,setOffsets: function() {
		samples_Sample.prototype.setOffsets.call(this);
	...
	,onKeyDown: function(window,keyCode,modifier) {
		...
		samples_Sample.prototype.onKeyDown.call(this,window,keyCode,modifier);
	}
	...
	,txtOutput: function(d,s,color,size,gap) {
		...
		while(_g1 < _g) {
			...
			if(letter == 10) {
				...
				this.peoteView.setElement({ element : i, program : 0, displaylist : d, x : 4 + px, y : 3 + py, w : size, h : size, image : 0, tile : letter, rgba : color});
				px += size + gap;
