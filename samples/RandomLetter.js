...
ApplicationMain.create = function() {
	...
	ApplicationMain.app = new samples_RandomLetter();
...
var samples_RandomLetter = function() {
	samples_Sample.call(this);
};
samples_RandomLetter.__name__ = true;
samples_RandomLetter.__super__ = samples_Sample;
samples_RandomLetter.prototype = $extend(samples_Sample.prototype,{
	run: function() {
		var _g2 = this;
		this.startTime = haxe_Timer.stamp();
		this.peoteView = new de_peote_view_PeoteView({ maxImages : 2});
		var w = 34;
		var h = 20;
		var s = 30;
		...
		this.peoteView.setDisplaylist({ displaylist : 0, type : 8, maxElements : w * h, maxPrograms : 1, bufferSegments : 1, z : 0, enable : true});
		...
		timer.run = function() {
			...
			while(_g < w) {
				...
				while(_g1 < h) {
					...
					_g2.peoteView.setElement({ element : y * w + x, x : x * s, y : y * s - s, w : s, h : s, program : 0, image : 0, tile : Math.floor(Math.random() * 256), rgba : Math.floor(Math.random() * 34) << 24 | Math.floor(Math.random() * 55) << 16 | Math.floor(Math.random() * 256) << 8 | 128 + Math.floor(Math.random() * 128)});
				}
			}
			firstrun = false;
		};
	}
	,__class__: samples_RandomLetter
});
var $_, $fid = 0;
