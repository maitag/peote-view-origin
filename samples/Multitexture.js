...
ApplicationMain.create = function() {
	...
	ApplicationMain.app = new samples_Multitexture();
...
var samples_Multitexture = function() {
	samples_Sample.call(this);
};
samples_Multitexture.__name__ = true;
samples_Multitexture.__super__ = samples_Sample;
samples_Multitexture.prototype = $extend(samples_Sample.prototype,{
	run: function() {
		var _g = this;
		this.startTime = haxe_Timer.stamp();
		var t = haxe_Timer.stamp() - this.startTime;
		this.peoteView = new de_peote_view_PeoteView({ maxDisplaylists : 10, maxPrograms : 100, maxTextures : 100, maxImages : 1000});
		this.peoteView.createFramebuffer();
		...
		this.peoteView.imageCache.setImage({ image : 1, texture : 1, filename : "assets/peote_tiles.png"});
		this.peoteView.imageCache.setImage({ image : 2, texture : 2, filename : "assets/peote_tiles_bunnys.png", preload : true});
		this.peoteView.setProgram({ program : 0, textures : [0,1]});
		var timer = new haxe_Timer(2000);
		timer.run = function() {
			timer.stop();
			_g.peoteView.setProgram({ program : 0, textures : [0,2]});
		};
		this.peoteView.setDisplaylist({ displaylist : 0, type : 156, maxElements : 10000, maxPrograms : 10, bufferSegments : 10, x : 0, y : 0, w : 512, h : 512, z : 0, renderBackground : true, r : 0.1, g : 0.5, b : 0.8, a : 0.8, enable : true});
		this.peoteView.setElementDefaults({ displaylist : 0});
		this.peoteView.setElement({ element : 0, x : 100, y : 100, w : 100, h : 100, pivotX : 50, pivotY : 50, rotation : -90, program : 0, image : 0, tile : 2});
		this.peoteView.setElement({ element : 1, x : 200, y : 100, w : 100, h : 100, pivotX : 50, pivotY : 50, rotation : 135, program : 0, image : 1, tile : 1});
	}
	,onMouseDown: function(window,x,y,button) {
		var e = this.peoteView.pick(0,haxe_Timer.stamp() - this.startTime,this.mouse_x,this.mouse_y,this.zoom,this.xOffset,this.yOffset);
		if(e != -1) this.peoteView.setElement({ element : e, tile : this.peoteView.getElement({ element : e}).tile + 1});
	}
	,__class__: samples_Multitexture
});
var $_, $fid = 0;
