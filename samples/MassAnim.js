...
ApplicationMain.create = function() {
	...
	ApplicationMain.app = new samples_MassAnim();
...
samples_MassAnim.__name__ = true;
samples_MassAnim.__super__ = samples_Sample;
samples_MassAnim.prototype = $extend(samples_Sample.prototype,{
	run: function() {
	}
	,rundelayed: function() {
		...
		this.peoteView = new de_peote_view_PeoteView({ maxDisplaylists : 2, maxPrograms : 2, maxImages : 4});
		this.peoteView.setTexture({ texture : 0, slots : 2, w : 512, h : 512});
		this.peoteView.imageCache.setImage({ image : 0, texture : 0, filename : "assets/peote_font_green.png"});
		this.peoteView.imageCache.setImage({ image : 1, texture : 0, filename : "assets/peote_tiles.png"});
		this.peoteView.setProgram({ program : 0, fshader : "assets/lyapunov_01.frag"});
		this.peoteView.setProgram({ program : 1, texture : 0});
		this.peoteView.setDisplaylist({ displaylist : 0, type : 1, maxElements : 1, maxPrograms : 1, bufferSegments : 1, z : 0});
		this.peoteView.setDisplaylist({ displaylist : 1, type : 1, maxElements : this.w * this.h, maxPrograms : 1, bufferSegments : 1, z : 1});
		this.peoteView.setElement({ element : 0, displaylist : 0, time : t, x : 0, y : 0, w : 3000, h : 3000, program : 0, tw : 1000, th : 1000, end : { x : -1500, y : -1500, w : 11000, h : 11000, time : t + this.h / this.speed}});
		this.update_time = haxe_Timer.stamp();
		var _g1 = 0;
		var _g = this.w;
		while(_g1 < _g) {
			var x = _g1++;
			...
			while(_g3 < _g2) {
				var y = _g3++;
				this.peoteView.setElement({ element : y * this.w + x, displaylist : 1, start : { x : x * this.s, y : y * this.s - this.s, w : this.s, h : this.s, time : t}, end : { x : x * this.s, y : y * this.s - this.s + this.h * this.s, w : this.s, h : this.s, time : t + this.h / this.speed}, program : 1, image : Math.floor(Math.random() * 2), tile : 1 + Math.floor(Math.random() * 255)});
			}
		}
		this.timer = new haxe_Timer(Math.floor(1000 / this.speed));
		this.timer.run = $bind(this,this.moveTilesUp);
		null;
	}
	,render: function(renderer) {
		if(this.firstrun) {
			...
			this.frames++;
			samples_Sample.prototype.render.call(this,renderer);
		}
	}
	,moveTilesUp: function() {
		...
		while(_g1 < _g) {
			var x = _g1++;
			this.peoteView.setElement({ element : this.last_y * this.w + x, displaylist : 1, start : { y : -this.s, time : t}, end : { y : -this.s + this.h * this.s, time : t + this.h / this.speed}, program : 1, image : Math.floor(Math.random() * 2), tile : 1 + Math.floor(Math.random() * 255)});
		}
		if(this.last_y == 0) {
			this.last_y = this.h - 1;
